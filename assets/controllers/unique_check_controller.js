...
export default class extends Controller {
  ...
  async connect() {
    ...
    this.existingValues = data.filter(v => v.toLowerCase() !== ignore);
  }
  ...
  check() {
    ...
    const isDuplicate = this.existingValues.some(v => v.toLowerCase() === value);
  ...
  renderHelpText(matches) {
    ...
    let displayTags = matches.slice(0, 10).join(', ');
