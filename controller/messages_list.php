...
class messages_list
{
    public function messages_list(Request $request, app $app):Response
    {
        ...
        $show_visibility_column = !$app['s_guest']
            && $app['intersystems']->get_count($app['tschema']);
        ...
        if (!count($messages))
        {
            return self::no_messages($app['pagination'], $app['tpl'], $out);
        }
    ...
    public static function get_messages_and_params(Request $request, app $app):array
    {
        $filter = $request->query->get('f', []);
        $pag = $request->query->get('p', []);
        $sort = $request->query->get('s', []);
