...
class MollieStatusController extends AbstractController
{
    ...
    ):Response
    {
        ...
        if (!$payment)
        {
            throw new NotFoundHttpException('Betaling met id ' . $id . ' niet gevonden.');
        }

        if (!$su->is_owner($payment['user_id']))
        {
            throw new AccessDeniedHttpException('Je hebt geen toegang tot status van betaling met id ' . $id);
        }
